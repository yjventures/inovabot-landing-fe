    ...
    <>
      {t.features.map(feature => (
        <tr key={feature.name} className='border border-l-0 last:border-r-0'>
          <th scope='row' className='py-8 px-5 text-lg font-medium text-text-secondary leading-6 border border-l- 0'>
            {feature.name}
          </th>
          {t.tiers.map(tier => (
            <td key={tier.id} className='px-6 py-8 xl:px-8 border border-l-0 last:border-r-0'>
