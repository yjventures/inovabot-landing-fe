...
const features = [
  { name: 'Integrations', tiers: { Basic: true, Essential: true, Premium: true, Enterprise: true } },
  { name: 'Shared links', tiers: { Basic: true, Essential: true, Premium: true, Enterprise: true } },
  { name: 'Importing and exporting', tiers: { Essential: true, Premium: true, Enterprise: true } },
  {
    name: 'Team members',
    tiers: { Essential: 'Up to 20 users', Premium: 'Up to 50 users', Enterprise: 'Unlimited' }
  },
  { name: 'Advanced analytics', tiers: { Basic: true, Essential: true, Premium: true, Enterprise: true } },
  { name: 'Basic reports', tiers: { Essential: true, Premium: true, Enterprise: true } },
  { name: 'Professional reports', tiers: { Premium: true, Enterprise: true } },
  { name: 'Custom report builder', tiers: { Premium: true, Enterprise: true } },
  { name: '24/7 online support', tiers: { Basic: true, Essential: true, Premium: true, Enterprise: true } },
  { name: 'Quarterly product workshops', tiers: { Essential: true, Premium: true, Enterprise: true } },
  { name: 'Priority phone support', tiers: { Essential: true, Premium: true, Enterprise: true } },
  { name: '1:1 onboarding tour', tiers: { Premium: true, Enterprise: true } },
  { name: '24/7 online support', tiers: { Basic: true, Essential: true, Premium: true, Enterprise: true } },
  { name: 'Quarterly product workshops', tiers: { Essential: true, Premium: true, Enterprise: true } },
  { name: 'Priority phone support', tiers: { Essential: true, Premium: true, Enterprise: true } },
  { name: '1:1 onboarding tour', tiers: { Premium: true, Enterprise: true } }
...
export default function PricingComparison() {
  return (
    <div className='pb-20 container hidden lg:block'>
      {/* lg+ */}
      <div className='isolate border pt-20'>
              ...
              <tr>
                <th scope='row' className='p-3 space-y-3'>
                  <div className='flex flex-wrap items-center gap-x-4'>
                    <Typography variant='h6'>Compare Plans</Typography>
                    <button className='px-3 py-1 rounded-full text-text border border-text font-normal'>40% Off</button>
                  </div>
                  <p className='text-text-tartiary text-sm font-normal'>
                ...
                {tiers.map(tier => (
                  <td key={tier.id} className='px-6 py-5 xl:px-8 border border-t-0 space-y-6'>
                    <div className='flex items-baseline gap-x-1'>
                      ...
                      <span className='text-sm font-semibold leading-6 text-text-tartiary'>/month</span>
                    </div>
                    <Button className='w-full bg-black hover:bg-primary'>Choose This Plan</Button>
                  </td>
                ))}
              </tr>
              <>
                {features.map(feature => (
                  <tr key={feature.name} className='border border-l-0 last:border-r-0'>
                    <th
                      scope='row'
                      className='py-6 px-5 text-lg font-medium text-text-secondary leading-6 border border-l- 0'
                    >
                      {feature.name}
                      <div className='absolute inset-x-8 mt-4 h-px' />
                    </th>
                    {tiers.map(tier => (
                      <td key={tier.id} className='px-6 py-6 xl:px-8 border border-l-0 last:border-r-0'>
